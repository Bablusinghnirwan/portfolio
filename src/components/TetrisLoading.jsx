...
// Tetris pieces - adapted for dark theme visibility
const TETRIS_PIECES = [
    // I-piece
    { shape: [[1, 1, 1, 1]], color: 'bg-astral-blue' },
    // O-piece
    { shape: [[1, 1], [1, 1]], color: 'bg-astral-blue' },
    // T-piece
    { shape: [[0, 1, 0], [1, 1, 1]], color: 'bg-astral-blue' },
    // L-piece
    { shape: [[1, 0], [1, 0], [1, 1]], color: 'bg-astral-blue' },
    // S-piece
    { shape: [[0, 1, 1], [1, 1, 0]], color: 'bg-astral-blue' },
    // Z-piece
    { shape: [[1, 1, 0], [0, 1, 1]], color: 'bg-astral-blue' },
    // J-piece
    { shape: [[0, 1], [0, 1], [1, 1]], color: 'bg-astral-blue' },
]
...
export default function TetrisLoading({
            ...
            if (linesToClear.length > 0) {
                ...
                const newGrid = prevGrid.map((row, rowIndex) => {
                    if (linesToClear.includes(rowIndex)) {
                        return row.map(cell => ({ ...cell, color: 'bg-white animate-pulse opacity-50' }))
                    }
                    return row
                })
    ...
    return (
        <div>
            <div className="mb-6">
                <div className={`border-2 border-white/20 bg-black/50 backdrop-blur-sm ${config.padding} transition-colors`}>
                    {renderGrid()}
                </div>
            </div>
            ...
            {showLoadingText && (
                <div className="text-center">
                    <p className="text-astral-blue font-outfit font-medium transition-colors animate-pulse">{loadingText}</p>
                </div>
            )}
        </div>
    )
